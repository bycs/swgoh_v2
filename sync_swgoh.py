...
def get_units_player(ally_code):
  ...
  units = pd.json_normalize(requests.get(link).json(),
                           'units', [['data', 'name'], ['data', 'ally_code']],
                           record_prefix='unit.', max_level=2,)
  units = pd.DataFrame(
      units, index=None, columns=['data.ally_code', 'data.name',
      'unit.data.base_id', 'unit.data.name', 'unit.data.rarity',
  ...
  #units.loc[:, 'unit.data.rarity':'unit.data.relic_tier'] = units.loc[:, 'unit.data.rarity':'unit.data.relic_tier'].astype('int8')
  #units.loc[:, 'unit.data.power':'unit.data.stats.28'] = units.loc[:, 'unit.data.power':'unit.data.stats.28'].astype('int32')
  #units.loc[:, 'unit.data.stats.5':'unit.data.stats.6'] = units.loc[:, 'unit.data.stats.5':'unit.data.stats.6'].astype('int16')
  #units.loc[:, 'unit.data.stats.16':'unit.data.stats.18'] = units.loc[:, 'unit.data.stats.16':'unit.data.stats.18'].astype('float16')
  return units
